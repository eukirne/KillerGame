...
router.get('/', requireAuth, (req, res) => {
  const groups = db
    ...
    .all(req.userId);
  res.json({ groups: groups.map((g) => groupSummary(g, req.userId)) });
});
...
router.post('/', requireAuth, (req, res) => {
  ...
  res.status(201).json({ group: groupSummary(group, req.userId), invited, notFound });
});
...
router.get('/:id', requireAuth, (req, res) => {
  ...
  const members = memberIds.map((id) => publicUser(getUserById(id)));
  const net = getGroupNetPositions(groupId, memberIds);
  const netOut = {};
  for (const id of memberIds) netOut[id] = fromCents(net.get(id) || 0);

  const pairwise = getGroupPairwiseBalances(groupId, memberIds).map((p) => ({
...
router.put('/:id', requireAuth, (req, res) => {
  ...
  res.json({ group: groupSummary(updated, req.userId) });
});
